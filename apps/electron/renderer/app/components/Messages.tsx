...
import { DeltaBlock, ImageBlock, TextBlock, ToolBlock } from '@uaito/sdk';
...
export const MessageItem:React.FC<{
    ...
    if (content.type === "text") {
        return <MessageContainer id={id} isUser={isUser}>
            <Markdown searchText={searchText} children={content.text} />
        </MessageContainer>
        ...
        return <ToolComponent name={content.name} messageId={`msg-${id}-block`} {...content} />
    }else  if (content.type === "tool_start") {
        return <ToolComponent name={content.name} messageId={`msg-${id}-block`} {...content} />
    } else if (content.type === "tool_result") {
        const toolName = app.user.chats.reduce<null | string>((name, chat) => {
            const index = chat.messages.findIndex((m) =>
                m.type === "tool_use" &&
                m.id === content.tool_use_id);

            if (index >= 0) {
                return (chat.messages[index] as any).name
            }
            return name
        }, null)
        return <ToolComponent name={toolName} messageId={`msg-${id}-block`} {...content} />
    }
}
