...
export const nodeIgnorePlugin = {
  name: "node-ignore",
  setup(build: any) {
    build.onResolve({ filter: /\.node$/ }, (args: any) => {
      return { path: args.path, namespace: "node-ignore" };
    });
    build.onLoad({ filter: /.*/, namespace: "node-ignore" }, () => {
      return {
...
export default function createConfig({
  ...
  return defineConfig(({ watch: _watch }) => ({
    ...
    clean: false,
    esbuildPlugins: [wasmPlugin, nodeIgnorePlugin, ...plugins],
    banner,
