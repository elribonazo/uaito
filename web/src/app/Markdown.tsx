...
export const CodeBlock = ({ searchText, node, inline, className, children, ...props }:any) => {
  const match = /language-(\w+)/.exec(className || '');
  const highlightedCode = String(children).replace(/\n$/, '');

  const renderComponent = (component:any, i:any) => {
...
const processMessage = (message:any, search:any) => {
  if (!search.length) return message;
  const regex = new RegExp(search, 'gmi');
  return message.replace(regex, (match:any) => `<mark style="background-color: yellow;">${match}</mark>`);
}

export function Markdown({ children, searchText }:any) {
  return (
    <ReactMarkdown
      components={{
